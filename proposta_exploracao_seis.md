...
## Experimento manipulado para obtenção de significância estatística
  ...
  - Para a obtenção de significância estatística na análise da amostra, uma parte dos dados da planilha mencionada anteriormente foi manipulada da seguinte forma: inicialmente, a tabela de resultados do projeto Math, associada à heurística Ochiai, foi modificada mediante o acréscimo do valor 10 à métrica Pos-Fault para a técnica e81_smote, exclusivamente nos casos de empate, isto é, nas situações em que os valores de Pos-Fault eram idênticos entre as técnicas e80_original e e81_smote. A adoção desse procedimento de manipulação dos dados fundamenta-se em evidências apresentadas em pesquisas realizadas, que indicam que um dos principais fatores que dificultam a obtenção de significância estatística é o elevado equilíbrio (empates ou pequena diferença) entre os valores comparados. A relação entre essa manipulação e seu impacto na análise pode ser melhor compreendida por meio das tabelas apresentadas a seguir, que mostram a estrutura original dos dados e a versão resultante após o ajuste realizado.

![Wilcoxon Signed - e80_original e e81_smote](img/Explicacao__Wilcoxon_Signed_e80_e81.png)
